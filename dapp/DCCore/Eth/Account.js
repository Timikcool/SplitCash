...
export default class Account {
  ...
  sign (raw) {
    Utils.debugLog(
      ['call %web3.eth.accounts.sign', ['font-weight:bold;']],
      _config.loglevel
    )
    Utils.debugLog(
      'More docs: http://web3js.readthedocs.io/en/1.0/web3-eth-accounts.html#sign',
      _config.loglevel
    )
  ...
  checkHashSig (rawMsg, signedMsg, needAddress) {
    return (
      needAddress.toLowerCase() ===
      ETHLib.recover(rawMsg, signedMsg).toLowerCase()
    )
  }
  ...
  async sendBets (toInput, amountInput, callback = false) {
    ...
    return this._ERC20.methods
      ...
      .send({
        ...
        gas: await this._ERC20.methods
          .transfer(to, amount)
          .estimateGas({ from: this.get().openkey })
      })
