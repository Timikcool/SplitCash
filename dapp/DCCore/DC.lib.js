...
export default class DCLib {
    ...
    E.on('_ready', () => {
      ...
      if (typeof document !== 'undefined') {
        document.dispatchEvent(
          new CustomEvent('DCLib::ready', { detail: this.config })
        )
      }
    })
  ...
  randomHash (data) {
    if (
      ...
      throw new Error(
        'Invalid data for randomHash, need: {bet:100, gamedata:array} '
      )
    }
  ...
  checkSig (rawMsg, signedMsg, needAddress) {
    ...
    return (
      ...
      this.web3.eth.accounts.recover(rawMsg, signedMsg).toLowerCase()
    )
  }
